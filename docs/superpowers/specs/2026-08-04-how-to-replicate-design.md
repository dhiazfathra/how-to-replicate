...
One append-only log holds console, network, interaction, and navigation events in
...
Three sources, one table. Playback scrubbing, step generation, and MCP export all
read one ordered source instead of joining three and reconciling three notions of
...
chain. Every provider failure is non-fatal: the capture stays `ready` with
deterministic steps. AI is convenience; redaction is compliance. Only the latter
blocks.
...
**Every LLM-authored step must cite `eventIds` that exist in the timeline. Steps
citing unknown IDs are dropped before the document is stored.** Not flagged —
dropped. If more than half a document's steps fail validation, the whole LLM pass
is discarded and the deterministic document stands.

An engineer must be able to trust that every step is backed by a real recorded
event. That guarantee is worth more than fluent prose.
...
- **Phase 1+:** LRU over captures where `sync.lastPushedAt !== null`. A synced
  capture is safe to evict; the server has it.
...
- **Transport:** ConnectRPC over HTTP for mutations and uploads; WebSocket for
  delta fan-out.
- **Push:** append-only mutation log, `{id: ULID, captureId, field, value,
  clientTs}`, batched and flushed by the sync engine. Idempotent on mutation ULID,
  so a retry after an ambiguous failure is always safe.
- **Pull:** `since=<revision>` delta stream. The server owns revision numbering.
- **Assets:** presigned PUT straight to object storage; a capture reaches `synced`
  only when its manifest is complete.
- **Offline:** the mutation queue lives in IndexedDB and survives restart. A week
  offline then a reconnect is a normal, tested path — not an edge case.
...
| Provider | Flow | Why |
|---|---|---|
| GitHub | OAuth **device flow** (`/login/device/code`) | GitHub does not support PKCE for public clients. Device flow is the supported secretless path — and needs no redirect URI, which suits an extension. |
| GitLab | OAuth **PKCE** | GitLab supports PKCE for public clients. |
...
straight into `sync-gateway`, no download step. Server-side redaction audit runs
before the capture is viewable, because an anonymous external reporter's machine is
not a trust boundary we control.
